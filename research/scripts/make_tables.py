...
def _wrap(
    ...
    ``wide=True`` uses ``table*`` (spans both IEEEtran columns, floats to the top of a page)
    instead of ``table`` -- for a table whose column count/content cannot fit one column's
    width at any reasonable font size, not a default either.
    """
