...
| `run_dedup.py --corpus cf --sweep` | **Recall only** for the three zero-marginal-cost matchers × 5 severities × 3 seeds; the cascade at 3 severities spanning the same range × 1 seed, adjudicating every pair in its band. **Precision and F1 are computed and stored in the run record for audit but never surfaced** — Corpus B's negative set carries measured contamination (amendment 8) that makes precision uninterpretable there. This figure has no Corpus A panel; Corpus A's dedup result is the separate, unswept `T4_abtbuy.tex` | `F1_severity.pdf` |
...
| `run_transfer.py` | **the External Validation comparison.** Thresholds selected on the Corpus A dev partition, carried across **unchanged**, evaluated on degraded Corpus B. Reports **recall and pair completeness** (not precision or F1, which Corpus B's negative-set contamination makes uninterpretable, amendment 8) and their difference as one paired comparison | `T9_transfer.tex` |
...
| 7 | **Distractor contamination is a corpus property, not a mining-rule defect, and full hand-verification of a bounded pool was replaced by a 50-pair random sample with a reported rate.** Contracts Finder publishes one procurement as several notices, so a high-similarity pair is at risk of being a hidden duplicate whether the mining rule proposed it or blocking merely surfaced it as a candidate — exhaustively verifying a small `max_pairs`-bounded pool never certified the *unbounded* pool actually used downstream against that risk, it only certified itself. `annotation/judge_distractors.py` now draws 50 pairs uniformly at random from the mined pool (unbounded except for a mining-cost cap) and reports the contamination rate with a Wilson score 95% interval; **the pool itself is used by C6 and the transfer runner unfiltered** — no pair is dropped on the strength of a judgement. `degrade.py`'s `make_distractors` docstring comment on `max_pairs`, which stated the old verification-bound rationale, is corrected to describe it as a mining-cost cap. Downstream: Corpus B precision is reported as a lower bound, with the contamination rate and its interval stated alongside — never used to arithmetically correct the precision figure. **Superseded by amendment 8 below**: the measured rate turned out high enough that a lower-bound framing does not hold either — Corpus B carries no precision figure at all. The Discussion records what this costs: without a verified negative set, the in-domain duplicate-detection result on Corpus B carries a known, quantified impurity; Corpus A is unaffected, its labels being published by others. The 48%→35% mining-rule story (`audit_distractors.py`) is unchanged and unaffected — it is a different measurement, of the mining rule's own correction, not of what remains after it | §6.6, §11 (no formal task — this tool has never had one), `annotation/judge_distractors.py`, `main.tex` §Methodology |
| 8 | **The measured rate settles what Corpus B can support: no precision figure, on any script, ever.** The 50-pair sample (amendment 7) judged by Claude — not the Groq adjudicator under test, so the model being measured never sits on both sides of the comparison — came back at 42.0% contaminated (Wilson 95% CI 29.4%–55.8%, n=50, `annotation/labels/distractor_judgements.jsonl`). Contamination in a negative set leaves recall untouched (recall is computed over duplicates whose membership is known by construction) but corrupts precision directly: a matcher that correctly calls a contaminated pair a duplicate is scored as a false positive, and the penalty scales with how good the matcher is. **Precision and F1 are therefore not reported on Corpus B by any runner** — not as a headline, not as a lower bound. `run_dedup.py --corpus cf --sweep` still computes and stores precision/F1 in the run record (auditable, never deleted), but `make_tables.py` must refuse to surface them into any table or figure for that corpus; only recall crosses into `F1_severity.pdf`. `run_transfer.py`'s External Validation comparison is recall and pair completeness only. `T3_blocking.tex` is unaffected (no precision column ever). **Open, not resolved here**: `run_operating_point.py` is specified (§10) to consume "the severity sweep" as one of its three inputs, and the only severity sweep in the plan is Corpus B's — but `main.tex` now states the RQ3 operating point is carried by the benchmark corpus. Either a new `run_dedup.py --corpus abtbuy --sweep` is in scope (new budget, not currently costed under §10.1), or "the severity sweep" `run_operating_point.py` consumes was never meant to be Corpus B's and the dependency line in §10 needs correcting — this is the supervisor's call, not resolved in this amendment | §10, §11 (no formal task), `main.tex` §Methodology, §Evaluation Metrics, §External Validation |
| 9 | **The Corpus A sweep is in scope, and the cascade belongs to it.** Amendment 8 left open which severity sweep `run_operating_point.py` consumes. Resolved by the supervisor in favour of the paper: **the plan is wrong, not the paper.** §10 omitted a Corpus A sweep and placed the cascade inside the Corpus B sweep, while `main.tex` states that RQ3's operating point is carried by the benchmark corpus and that precision is not reportable on the procurement one — so the cascade, whose whole result is a trade of precision against cost, could only ever have run on Corpus A. `run_dedup.py` gains `--corpus abtbuy --sweep`, and `dedup.yaml`'s `cascade.corpus` is fixed at `abtbuy`. Costed before building rather than after: three severities `[0.0, 0.15, 0.5]` at one repetition, with severity 0.5 subsampled to `m=800` stratified by label, fits the free-tier allowance in ~5 days against 6.1 for the full band. **Consequences beyond the sweep**: `run_operating_point.py` consumes the Corpus A sweep; `run_transfer.py` fits on Corpus A and carries across to Corpus B unchanged; `F1_severity.pdf` keeps no Corpus A panel, as specified | §10, §10.1, §11 (G9, G10), `research/configs/dedup.yaml`, `main.tex` §Evaluation Metrics |
| 10 | **Handling time is not measured anywhere, and residual effort is review volume.** RQ3 previously converted an automated share into hours using `mean_seconds_per_item` from a timed annotation exercise. Removed: an author timing their own reading measures that author, and a model timing its own measures endpoint latency; reporting either as staff curation effort would not be a limitation but a false measurement. `operating_point.residual_effort` takes **no** handling-time argument — there is no parameter to pass one into — and returns review volume with `EFFORT_FORMULA` beside it, so a reader holding a figure for their own institution converts and a reader without one still has the quantity the automated share determines. Nothing is lost, because the reduction is a ratio and is the same number whatever the handling time turns out to be. `annotate.py --timing-only` and the `mean_seconds_per_item -> operating_point` hand-off are deleted; `timing.py` survives as instrumentation only. The review queue still records per-decision duration, as deployment telemetry rather than as an input to a result | §6.13, §6.15, §10, §11 (G9), `main.tex` §Evaluation Metrics |
| 11 | **The automated share is a two-bound quantity, and it is never reported without the duplicates it cost.** Two corrections at once. First, modelling automation as a single threshold — resolve above it, review everything below — scored every obvious non-duplicate as outstanding human work and put the automated share at 1.7% where the two-bound rule gives 93.4%; the cascade's accept/reject/band structure was already correct and is now shared as `operating_point.band_operating_point`. Second, an automated share alone is not reportable: clearing the field discards duplicates that never reach a reviewer, and the two errors are asymmetric — a wrong acceptance is a false merge a reader can see and undo and the precision floor bounds it, while a wrong rejection leaves a duplicate in the register indistinguishable from a genuine second item, unbounded and unexamined. Every operating point therefore carries `n_duplicates_lost` (in counts as well as rates) and `recall_ceiling`, the recall still reachable before the adjudicator acts. Measured on Corpus A at severity 0: 84 of 206 duplicates discarded at floor 0.95 against 16 at 0.99, ceilings 0.592 and 0.922 | §6.13, §10, `main.tex` §Evaluation Metrics |
| 12 | **A division-only CPV code is not a class.** `cpv_class` truncated `30000000` to `"3000"`, which is not a CPV class and has no taxonomy row; 22.6% of the Corpus B development partition and 15.3% of the test partition are published at division granularity. Scoring them at class level asks whether a classifier can predict that a buyer was vague — a property of publishing practice, not of the equipment — and credits it for reproducing a non-answer. `cpv.labelled_at` routes them to review instead. Class-level evaluation is now restricted on **two grounds reported apart**: 10.5 points for labels below `min_examples`, 15.3 points for records carrying no class at all. Class macro F1 moves 0.508 → 0.560 (`tfidf_svm`) and 0.351 → 0.410 (`embedding_logreg`); the superseded figure is measured alongside rather than deleted | §6.5, §6.10, §10, C7, `main.tex` §Category Assignment |
| 13 | **The shortlist is dropped at class level; the supported set is presented whole.** Measured before spending quota: the embedding retriever the paper specified puts the true code in its top 12 for only 68.2% of class-level test records on the 74-code supported pool, and 26.0% against the full 1,209-class taxonomy. A language model handed a shortlist without the answer cannot be right, so the condition's accuracy would have measured the retriever under the model's name. The supported set is 74 codes and ~756 tokens, so "presenting a taxonomy of this size in a prompt is neither affordable nor useful" is true of 6,424 leaves and false here. Retrieval is retained for the in-context examples, where an unhelpful example costs accuracy, and removed for the label set, where an absent label forecloses it. **Shortlist recall at k is reported as a result** — including the 26.0% full-taxonomy row, as the ceiling a deployment over the real vocabulary would face — with the `tfidf_svm` reference row and the caveat that shortlisting with the classifier RQ2 exists to beat would leave the model able only to re-rank the baseline. Sample sizes split by level: **class n=350, division costed separately** | §6.10, §10.1, C7, `main.tex` §Category Assignment |

**`G1` collides with the Phase G build-order task IDs and should not be extended.** It was
chosen to evoke "Groq," but every letter A–G is already a §11 phase prefix, and Phase G's own
task G2 (`audit_real_errors.py` + `run_degrade_check`) is a real, referenced ID —
`session-knowledge.md`'s "(`audit_real_errors.py`, G2)" means that, not an amendment. `G1`
stays as committed, since its row content is unambiguous in context and rewriting a shipped
ID creates its own confusion, but the entry immediately below it reverts to the plain-number
sequence rather than continuing as `G2`. Future amendments: do not use A–G as a prefix.
