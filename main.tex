...
into the training, validation and test splits supplied with it.
...
embedding model and classifies with a logistic regression head. A zero
shot approach presents the record and the candidate taxonomy to a large
language model without labelled examples. A retrieval augmented approach
retrieves the nearest labelled examples from the development partition
by embedding similarity and supplies them as in context examples,
testing whether the labels already available in Corpus B can be reused
...
further tuning, on degraded records from Corpus B. Corpus A carries
human labels but a commercial domain; Corpus B carries a domain closer
to institutional equipment but synthetic duplicate labels. Neither is a
...
annotation protocol, experimental code and analysis notebooks are
released in a public repository. Corpora derived from third party
