...
arising from distinct procurement processes. Distinctness is established
by the procurement reference rather than by the record identifier: the
publisher mints an identifier per notice rather than per process, so two
notices describing one procurement carry different identifiers, and a
rule keyed on them admits genuine duplicates into the negative set. Pairs
sharing a buyer and an otherwise identical title are excluded on the same
grounds. On the benchmark corpus distractors are drawn from pairs sharing
