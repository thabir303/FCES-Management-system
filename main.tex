...
register directly and the remainder enter a review queue. The system is
therefore both the destination of the migration and the setting in which
its residual human cost is observed, which is what allows that cost to be
reported as a measured quantity rather than an estimate.
...
surviving set is then verified by hand, with the contamination rate
measured at each stage and reported. The distractor set is bounded at a
size that admits complete manual verification, since a negative set of
unknown purity is worse than a smaller one of known purity. On the benchmark corpus distractors are drawn from pairs sharing
